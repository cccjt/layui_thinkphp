...
layui.define('view', function(exports){
  ...
  var events = admin.events = {
    ...
    ,note: function(othis){
      ...
      ,note;
      layui.admin.req({
        url:'system/note/getNote',
        data:{uid: layui.myUtils.cache('session').uid},
        done:function (res) {
           note = res.data.note;
           events.note.index = admin.popup({
            ...
            ,content: '<textarea placeholder="内容">'+note+'</textarea>'
            ,resize: false
            ,success: function(layero, index){
              var textarea = layero.find('textarea');
              layero.children('.layui-icon').click(function(event) {
                  var v = $.trim(textarea.val());
                  if(v!=''){
                      layui.admin.req({
                        url:'system/note/saveNote',
                        type:'post',
                        data: {note:v, uid: layui.myUtils.cache('session').uid}
                      })
                  }
              });
            }
